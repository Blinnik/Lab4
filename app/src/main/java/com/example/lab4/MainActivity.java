...
public class MainActivity extends AppCompatActivity {
    private static final String PREFS_NAME = "com.example.lab4.MainActivity";
    private static final String PREFS_PREFIX_KEY = "appwidget_";
...
//                    error.setText("Событие должно быть в будущем!");
                    ...
                    Days_num.updateAppWidget(context, appWidgetManager, mAppWidgetId);
    ...
    static Boolean isDone(Context context, int appWidgetId) {
        ...
        return prefs.getBoolean(PREFS_PREFIX_KEY + appWidgetId + "b", false);
    }

    static void setDone(Context context, int appWidgetId, boolean done) {
        SharedPreferences.Editor prefs = context.getSharedPreferences(PREFS_NAME, 0).edit();
        prefs.putBoolean(PREFS_PREFIX_KEY + appWidgetId + "b", done);
        prefs.apply();
    }
    static void saveDatePref(Context context, int appWidgetId, String text, boolean done, boolean shown) {
        SharedPreferences.Editor prefs = context.getSharedPreferences(PREFS_NAME, 0).edit();
        prefs.putString(PREFS_PREFIX_KEY + appWidgetId, text);
        prefs.putBoolean(PREFS_PREFIX_KEY + appWidgetId + "b", done);
        prefs.putBoolean(PREFS_PREFIX_KEY + appWidgetId + "n", shown);
        prefs.apply();
    }
    ...
    static void deleteDatePref(Context context, int appWidgetId) {
        ...
        prefs.remove(PREFS_PREFIX_KEY + appWidgetId + "b");
        prefs.remove(PREFS_PREFIX_KEY + appWidgetId + "n");
        prefs.apply();
    }
    ...
    static String loadDatePref(Context context, int appWidgetId) {
        SharedPreferences prefs = context.getSharedPreferences(PREFS_NAME, 0);
        String titleValue = prefs.getString(PREFS_PREFIX_KEY + appWidgetId, null);
        if (titleValue != null) {
            return titleValue;
        } else {
            return "";
    ...
    static Boolean wasNotifShown(Context context, int appWidgetId) {
        SharedPreferences prefs = context.getSharedPreferences(PREFS_NAME, 0);
        return prefs.getBoolean(PREFS_PREFIX_KEY + appWidgetId + "n", false);
    }
